...
// Userconfig
[] call (compile (preprocessFileLineNumbers "\userconfig\btk\btk_clientsettings.hpp"));
...
if (isNil "btk_clientsettings_enabled") then {
	...
	[] spawn { sleep 1; hint parseText format["<t align='left'><t color='#ff0000'>WARNING</t><br />BTK Client Settings userconfig is missing!<br />Using default settings...</t>"]; };
};
...
// Variables
btk_clientsettings_viewdistance_min = 1000; // Min viewdistance
...
[] spawn {
	...
	while {true} do {
		...
		// Dialog open?
		if (isNil "btk_clientsettings_dialog_open") then {
